...
Imager.prototype = {
  ...
  pushToLocal: function (tempFile, localFile, filename, type, cb) {
    ...
    var localPath = path.resolve(path.join( // find destination path
      this.config['storage'][this.storage].path, directory, localFile
    ));
    var mode = this.config['storage'][this.storage].mode || 0777;
  ...
  */
  ...
  getClientForRackspace: function (cb) {
    ...
    var client = this.clientCache[this.storage];
    if (!client) {
      ...
      if (!client) return cb('Unable to create client for ' + this.storage);

      client.containerCache = {};
      this.clientCache[this.storage] = client;
    }
