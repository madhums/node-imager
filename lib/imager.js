...
Imager.prototype = {
  ...
  resizeFile: function (file, preset, fname, cb) {
    ...
    var filename = fname + contentType[ct];
    var self = this;
    var remoteFile = preset.name + preset.sep + filename;
    var tempFile = tempDir + preset.name + '_' + filename;
    ...
    gm(file['path'])
      ...
      .write(tempFile, function (err) {
        ...
        self['pushTo' + self.storage](tempFile, remoteFile, filename, ct, cb);
      });
  },
  ...
  pushToRackspace: function (file, remoteFile, filename, type, cb) {
    ...
    var clientConfig = this.config['storage'][this.storage];
    ...
    if (clientConfig.auth) {
      for (var key in clientConfig.auth) {
        if (clientConfig[key]) continue; // skip it
    ...
    var client = pkgcloud.storage.createClient(this.config['storage'][this.storage]);
    ...
    fs.readFile(file, function (err, buf) {
      ...
      var addFileToContainer = function (err, container) {
        ...
        client.upload(options, onUploadToContainer);
      };

      client.getContainer(clientConfig.container, function (err, container) {
        if (err && err.statusCode === 404) {
          log('Creating container ' + (clientConfig.container.name || clientConfig.container));
          return client.createContainer(clientConfig.container, addFileToContainer);
        } else return addFileToContainer(err, container);
      });
    });
  },
  ...
  removeFromRackspace: function (file, preset, cb) {
    ...
    // this maintains compatibility with v0.1.12 configuration files
    if (clientConfig.auth) {
      for (var key in clientConfig.auth) {
        if (clientConfig[key]) continue; // skip it
        clientConfig[key] = clientConfig.auth[key];
      ...
      clientConfig.authUrl = clientConfig.host;
      if (clientConfig.authUrl.indexOf('http') !== 0) {
        clientConfig.authUrl = 'https://' + clientConfig.authUrl;
      }
    }
